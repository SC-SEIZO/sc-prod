...
async function run() {
  ...
  if (error) {
    console.error('Error fetching users (probably column username or password_hash is missing):');
    console.error(error);
    console.log('\n--> ACTION REQUIRED: Please execute migration_users_username.sql in your Supabase SQL Editor.');
    process.exit(1);
  }
  ...
  for (const user of seedUsers) {
    ...
    if (!existing) {
      console.log(`Seeding missing user: ${user.username} (${user.role})...`);
